					...
					{if isset($images) && count($images) > 0}
						...
						<div id="views_block" class="clearfix {if isset($images) && count($images) < 2}hidden{/if}">
							...
							<script>
								var galleryList = {};
								$(function(){
									galleryList.box = $('ul#thumbs_list_frame')[0];
									galleryList.countImg = $('ul#thumbs_list_frame li').size();
									galleryList.countLines = Math.ceil(galleryList.countImg / 3);
									galleryList.lineHeight = 140;
									galleryList.up = function () {
										galleryList.box.scrollTop = galleryList.box.scrollTop - galleryList.lineHeight;
										galleryList.render();
									};
									galleryList.down = function() {
										galleryList.box.scrollTop = galleryList.box.scrollTop + galleryList.lineHeight;
										galleryList.render();
									};
									galleryList.render = function() {
										if(galleryList.box.scrollTop == 0){
											$('#r-arrow-top').hide();
										}else{
											$('#r-arrow-top').show();
										}

										if(galleryList.box.scrollTop + galleryList.box.clientHeight == galleryList.box.scrollHeight){
											$('#r-arrow-bottom').hide();
										}else{
											$('#r-arrow-bottom').show();
										}
									...
									$('#r-arrow-bottom').on('click', galleryList.down);
									$('#r-arrow-top').on('click', galleryList.up);
								});
							</script>
							<div id="thumbs_list">
								<div id="r-arrow-top" class="r-arrow"></div>

								<div id="r-arrow-bottom" class="r-arrow"></div>
										...
										{foreach from=$images item=image name=thumbnails}
											...
											<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
												<a{if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}',largeimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}'{literal}}{/literal}"{else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
													<img class="img-responsive" id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds)|escape:'html':'UTF-8'}" alt="{$imageTitle}" title="{$imageTitle}"{if isset($cartSize)} height="{$cartSize.height}" width="{$cartSize.width}"{/if} itemprop="image" />
												</a>
											</li>
										{/foreach}
									{/if}
