...
{if count($categoryProducts) > 0 && $categoryProducts !== false}
    <h3 class="col-md-12">
		{if $categoryProducts|@count == 1}
			{l s='%s other product in the same category:' sprintf=[$categoryProducts|@count] mod='productscategory'}<span style="text-transform:lowercase;">&nbsp;{$category->name|escape:'html':'UTF-8'}</span>
		{else}
			{l s='Другие модели в категории' sprintf=[$categoryProducts|@count] mod='productscategory'}{l s=' '}<span style="text-transform:lowercase;">{$category->name|escape:'html':'UTF-8'}</span>
		{/if}
	</h3>
	<div id="productscategory_list" class="clearfix">
		...
		{foreach from=$categoryProducts item='categoryProduct' name=categoryProduct}
			<li class="product-box item">
				<a href="{$link->getProductLink($categoryProduct.id_product, $categoryProduct.link_rewrite, $categoryProduct.category, $categoryProduct.ean13)}" class="lnk_img product-image" title="{$categoryProduct.name|htmlspecialchars}"><img src="{$link->getImageLink($categoryProduct.link_rewrite, $categoryProduct.id_image)|escape:'html':'UTF-8'}" alt="{$categoryProduct.name|htmlspecialchars}" /></a>
				<!-- Название товара
